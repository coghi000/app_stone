...
def list_gdp_region(region: str, db: Session = Depends(get_db)):
...
def list_gdp_year(country_name: str, db: Session = Depends(get_db)):
...
@app.get("/rank_pib/")
def list_gdp_year(year_ini: str, year_end: str, db: Session = Depends(get_db)):
